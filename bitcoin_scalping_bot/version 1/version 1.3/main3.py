...
def main(model_name, risk_adverse, epochs = 100, transaction=0.0002):
    ...
    env = Environment3(df, list(df_index.index), risk_adverse = risk_adverse, transaction=transaction, max_leverage=10)
    state = env.reset()
    ...
    for n_epi in tqdm.tqdm(range(epochs)):
        ...
        while not done:
            ...
            log.to_csv("bitcoin_scalping_bot\\version_3\\train_log\\log_{}.csv".format(n_epi+1))
...
if __name__ == '__main__':
    main(model_name="ppo3", risk_adverse=3, epochs=300, transaction=0.000)